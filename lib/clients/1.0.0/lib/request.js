...
class Request extends Events {
  ...
  handleData() {
    this._request.on('data', async (data) => {
      ...
      for (let line of lines) {
        ...
        if (json.error) {
          ...
          switch (error.code) {
            case -1150:
            case -1155:
              await this.render.authRenew();
              break;
            default:
              this.render.notifyError(error);
              this.logger.error('Receive error, code: %d, message: %s', error.code, error.message);
          }
          return;
        }
  ...
  businessWithdraw(elements, asset, address, amount) {
    const packet = {
