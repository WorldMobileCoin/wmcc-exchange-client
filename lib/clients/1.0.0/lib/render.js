...
//----------------------------
    ...
    //this._methods.set('asset.query', this._assetQuery.bind(this));


    //this._methods.set('depth.query', this._depthQuery.bind(this));
    ...
    //this._methods.set('today.query', this._todayQuery.bind(this));
    //this._methods.set('today.subscribe', this._todaySubscribe.bind(this));
    //this._methods.set('today.update', this._todayUpdate.bind(this));
  ...
  _authSignature(json) {
    const {result, loading, wrapper} = this._elements.get('auth_register');
    loading.element.remove();
  ...
  _marketPanel(el, market, prec) {
    //const flip = JQuery(`<div id="trade-flip">`);
    const chart = JQuery(`<div id="market-chart">`);//new Chart();//Chart.create(market, Render.CHART.DEFAULT_PERIOD);//JQuery(`<div id="market-chart">`);
    ...
    //this._elements.set('trade_flip', flip);
    ...
    //flip.parent().removeClass('flipped');
    //this._flipY([chart, trading, depth.element]);
  }
  ...
  _chartFrame(wrapper, market) {
    ...
    for (let [name, time] of Object.entries(Render.CHART.PERIODS)) {
      ...
      period.on('click', () => {
        ...
        this.request.klineQuery(market, 0, now, time, frame.get(0));
    ...
    frame.get(0).onload = () => {
      ...
      frame.get(0).contentWindow.postMessage(Object.assign(dimension, {init: true, market: market}), "*");
      const now = Math.floor(new Date() / 1000);
      this.request.klineQuery(market, 0, now, Render.CHART.DEFAULT_PERIOD, frame.get(0));
      this.request.klineSubscribe(market, Render.CHART.DEFAULT_PERIOD);
    }
  }
/*
  _marketChart(market) {
    const wrapper = JQuery(`<div id="market-chart">`);
  ...
  _orderLimit(assets) {
    /*if (!assets.subscribed && this.request.isAuth()) {
      assets.subscribed = true;
      const market = `${assets.asset}/${assets.base}`;
      const end = Math.floor(Date.now()/1000);
      const start = end - Render.HISTORY.TIME_MAX;
      this.request.assetSubscribe([assets.asset, assets.base]);
      this.request.orderQuery(market, 0, Render.ORDER.PAGE_MAX, 'trade');
      this.request.orderHistory(market, start, end, 0, Render.HISTORY.PAGE_MAX);
      this.request.orderSubscribe([market]);
    }*/
  ...
  _balances(currentAsset) {
    ...
    this._slide(parent, cancel);
    /*const {panel, back} = this._elements.get("asset_panel");
    const header = JQuery("<div>");
    const body = JQuery("<div>");
    const hide = JQuery(`<span class="toggle">Hide Zero Balance</span>`);
    const table = new Table({
    ...
    for (let [ticker, balance] of Object.entries(assets)) {
      const trow = table.trow()[0];
      const deposit = JQuery(`<td id="${ticker}_deposit" name="_deposit"><a class="button-flat-dark">Deposit</a></td>`);
      const withdraw = JQuery(`<td id="${ticker}_withdraw" name="_withdraw"><a class="button-flat-dark">Withdraw</a></td>`);
      const trade = JQuery(`<td><a class="button-flat-dark">Trade</a></td>`);

      const total = new Decimal(balance.available);
      total.add(balance.freeze);

      const td = [
        `<td>${ticker}</td>`,
        `<td>${this._assets[ticker].fullname}</td>`,
        `<td class="balance_total">${total.toString()}</td>`,
        `<td class="balance_available">${balance.available}</td>`,
        `<td class="balance_freeze">${balance.freeze}</td>`,
        deposit,
        withdraw,
        trade
      ];
      ...
      const _td = JQuery(`<td colspan="${len}"></td>`);
      ...
      trade.on('click', () => {
        ...
        this._goToTrade(`${ticker}/WMCC`);
      });
    }

    header.html([hide, back]);
    body.html(table.render());

    panel.html([header, body]);

    setTimeout(() => {
      if (this._temp.currentAsset) {
        const el = JQuery(`#${this._temp.currentAsset}_deposit`);
        if (!el.length)
          return;

        this._temp.currentAsset = null;
        el.trigger('click');
        body.animate({
  ...
  _assetsPanel(currentAsset) {
    ...
    for (let [name, asset] of Object.entries(this._assets)) {
      ...
      const _available = JQuery(`<td value="${available}"${available === '0' ? 'class="zerodash">–': '>'+available}</td>`);
      const _freeze = JQuery(`<td value="${freeze}"${freeze === '0' ? 'class="zerodash">–': '>'+freeze}</td>`);
      const _total = JQuery(`<td value="${total}"${total === '0' ? 'class="zerodash">–': '>'+total}</td>`);

      const cb = (balance) => {
        if (!_available.length)
          return;

        const sum = new Decimal(balance.available).add(balance.freeze).toString();
        _available.html(balance.available);
        _freeze.html(balance.freeze);
        _total.html(sum);
      }

      if (!item.fn.includes(cb))
        item.fn.push(cb);//.bind(this);
  ...
  _ordersHistory(market) {
    ...
    const table = new Table({
      ...
      //id: `order-history_${market.replace('/','-')}`
    });
  ...
  _currentOrders(wrapper) {
    ...
    /*for (let [market, order] of this._orderMap.entries()) {
      if (Object.keys(order.records).length === 0)
        continue;

      for(let [id, record] of Object.entries(order.records))
        this._orderRecords[id] = record;
    }*/
  ...
  _assetList(json) {
    ...
    /*const parent = JQuery(`<div class="slide-top">`);
    const panel = JQuery(`<div id="balances">`);
    const back = JQuery('<a class="button">Back</a>');

    this._elements.set("asset_panel", {
      panel: panel,
      back: back
    });
  ...
  _assetUpdate(assets) {
    /*if (JQuery('#balances').length)
      this._assetPanel(assets);
    else
      this._assetTrading(assets);*/
...
/*
    const opened = this._elements.get('open_orders');
    if (opened.market !== order.market)
      return;

    this.request.orderQuery(opened.market, ...opened.page, 'trade');
    ...
    if (event === Render.ORDER_EVENT.FINISH) {
      setTimeout(() => {
        ...
        const end = Math.floor(Date.now()/1000);
        const start = end - Render.HISTORY.TIME_MAX;
        this.request.orderHistory(history.market, start, end+60*60, ...history.page, 'trade');
      }, 1000);
    }*/
  }
  ...
  _orderCancel(json, args) {
    ...
    //if (limit)
    //  this._recordOpenOrders(table, offset, limit, options);
    //else
    //  this._tradeOpenOrders(market, table);
  }
  ...
  /*_authRegister(wrapper) {
    ...
    const register = JQuery('<div>');
    const header = `<h1>Register</h1>`;
    const body = [
      `<span>Register new session for an address. Each address represent an individual trading account.</span>`,
      `<span>Select one of your address below to prove the ownership:</span>`
    ];
    const error = JQuery('<div class="result">');
    const select = this._addressList();
    const button = JQuery('<a class="button">Submit</a>');

    button.on('click', () => {
      if (!select.val())
        return;

      this.request.register(address, (new Date())/1000 - 3600);
    });
    ...
    this._elements.set('registration', elements);

    return register.html([header, body.join(''), error, select, button]);
  }

  _session(wrapper) {
    const session = JQuery('<div>');
    const header = `<h1>Active Session</h1>`;
    const body = `<span>Use existing session to login.</span><span>An address can have multiple sessions.</span>`;
    const error = JQuery('<div class="result">');
    const select = this._sessionList();
    const button = JQuery('<a class="button">Login</a>');
    ...
    this._elements.set('session', elements);

    return session.html([header, body, error, select, button]);
  }
...
/*
  _onOrderChange(price, amount, total) {
    price.on('change')
...
/*
  _stateQuery(json, market) {
    const item = this._elements.get('markets');
    const element = item.markets.get(market);

    let data;
    if (item.page === Render.PAGE.HOME)
      data = this._parseMarketHome(market, json.result);
    else if (item.page === Render.PAGE.TRADE)
      data = this._parseMarketTrade(market, json.result, item.active, element);

    element.find('td').each((idx, el) => {
      JQuery(el).html(data[idx]);
    });
  }
*/
  ...
  _businessReload(json, args) {
    ...
    submit.after(`<span>You have successfully reloaded ${amount} ${asset} to your account.</span>`);
  }
...
/*
  _depositReal(element, asset) {
    this.request.businessDeposit(asset, element);
  }
*//*
  _depositDemo(element, asset) {
    //const total = element.parent().prev().find(".balance_total").text();
    //const {balance} = this._assetMap.get(asset);
    //const amount = new Decimal(balance.total);
    const submit = JQuery(`<div class="button">Deposit</div>`);

    //if (amount.cmp(100) > -1)
    //  element.html(`<span>Your minimum balance amount must be below 100${asset} to reload.</span>`);

    submit.on('click', () => {
      this._loading(element);
*//*
      const deposit = this._elements.set('business_reload', {
        element: element,
        asset: asset
      });*//*
      this.request.businessReload(asset, element);
    });
...
/*
  _assetQuery(json) {
    console.error(json);
    const list = JQuery(`<div id="asset_list"></div>`);
  }

  _assetQuery(json) {
      this._assetUpdate(json);
  }
*/
...
/*
  _getAddrs(limit = 10, offset = 0) {
    limit = parseInt(limit);
    offset = parseInt(offset);

    const len = this._addresses.length;
    const end = (limit+offset) > len ? len : offset+limit;

    const addrs = [];
    for (let i=offset; i<end; i++)
      addrs.push(this._addresses[i].toAddress());
...
*/
  /*_todayQuery(json) {
    ...
    for(let market of json.result) {
      ...
      markets.splice(-1, 0, tr.join(''));
    }

    this._write(markets.join(''), 'append');
  }

  _todaySubscribe(json) {
    console.error(json);
  }
